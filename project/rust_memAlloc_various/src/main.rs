...
fn add_string_immutable_access(size: usize, method: i32, eltype: i32, mutability: bool) {
    println!("size: {}, method: {}, type: {}, mutability: {}", size, get_methodName(method), get_elementType(eltype), get_mutability(mutability));
    ...
    let (elapsed_access, correct) = access_string_test_immutable(&immutable, &source);
    
    let elapsed_total = start_init.elapsed().as_nanos();

    write_to_file(method, eltype, mutability, size , elapsed_init, elapsed_add, elapsed_access, elapsed_total, correct);
}
...
fn add_string_mutable_access(size: usize, method: i32, eltype: i32, mutability: bool) {
    ...
    let (elapsed_access, correct) = access_string_test_mutable(&mut distination, &source);
...
fn add_integer_immutable_access(size: usize, method: i32, eltype: i32, mutability: bool) {
    ...
    let (elapsed_access, correct) = access_integer_test_immutable(&immutable, &source);
...
fn add_integer_mutable_access(size: usize, method: i32, eltype: i32, mutability: bool) {
    ...
    let (elapsed_access, correct) = access_integer_test_mutable(&mut distination, &source);
...
fn get_vector_integer(size: usize) -> Vec<i32> {
    ...
    let mut source = Vec::with_capacity(size);
    for _i in 0..size {
        let num = dist.sample(&mut thread_rng());
        source.push(num as i32);
    }
    return source;
}

fn write_to_file(method: i32, eltype: i32, mutability: bool, size: usize, elapsed_init: u128, elapsed_add: u128, elapsed_access: u128,  elapsed_total: u128, correct: bool) {
    let output = format!("[RustVector]#{:?}#{:?}#{:?}#{:?}#{:?}#{:?}#{:?}#{:?}#{:?}\n", 
                        get_methodName(method), get_elementType(eltype), get_mutability(mutability), 
                        size, elapsed_init, elapsed_add, elapsed_access, elapsed_total, correct);
    println!("{}",output);
...
fn access_integer_test_mutable(distination :&mut Vec<i32>, source: &Vec<i32>) -> (u128 , bool){
...
fn access_integer_test_immutable(distination :&Vec<i32>, source: &Vec<i32>) -> (u128, bool) {
    let start_access = Instant::now();
    let len = distination.len();
    let mut sum: i64 = 0;
    for i in 0..len {
...
fn access_string_test_mutable(distination :&mut Vec<String>, source: &Vec<String>) -> (u128, bool) {
    let start_access = Instant::now();
    let len = distination.len();
    let mut sum: i64 = 0;
    for i in 0..len {
...
fn access_string_test_immutable(distination :&Vec<String>, source: &Vec<String>) -> (u128, bool){
    let start_access = Instant::now();
    let len = distination.len();
    let mut sum: i64 = 0;
    for i in 0..len {
...
fn one_by_one<T: Eq + Ord + Clone>(dst: &mut Vec<T>, src: &mut Vec<T>, size: usize) -> u128 {
