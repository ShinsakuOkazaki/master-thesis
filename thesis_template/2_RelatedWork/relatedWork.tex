...
Rc<T> enables value to have multiple owners, but the value should be immutable. Rx<T> is used in case of single thread. 
When the situation is multi-thread, Arc<T> is used. Arc<T> performs atomic operation, so it is more expensive than Arc<T>. 
