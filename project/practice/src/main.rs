...
fn create_id_numeric(arr: &[(String, Vec<String>)], map: &HashMap<String, usize>) -> Vec<(String, Vec<i32>)>{
    let n = arr.len();
    let mut res = Vec::with_capacity(n);
    let mut numeric;
    let mut pair;
...
fn create_numeric_vector(words: &[String], map: &HashMap<String, usize>) -> Vec<i32> {
    let n = words.len();
    let k = map.len();
    let mut res = vec![0; k];
    let mut key;
