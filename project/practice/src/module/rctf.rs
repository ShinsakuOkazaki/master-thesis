...
pub fn vectorize_x_with_rc(source: &[Vec<f64>]) -> Array<f64, Ix2>{
...
pub fn vectorize_y_with_rc(source: &[Rc<String>]) -> (Array<i32, Ix1>, HashMap<i32, Rc<String>>) {
    let n = source.len();
    ...
    let mut encode = 0;
    for i in 0..n {
        id1 = Rc::clone(&source[i]);
        id2 = Rc::clone(&source[i]); 
